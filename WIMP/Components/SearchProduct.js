...
export default function SearchProduct({ route }) {
   ...
   const searchedProducts = (value) => {
      return productsAPIData.map((product) => {
         if (value != '' && (product.UPC_Code == value || product.Product_Name.toLowerCase().indexOf(value.toLowerCase()) == 0 || product.Category_Name.toLowerCase().indexOf(value.toLowerCase()) == 0 || product.Manufacturer_Name == value)) {
