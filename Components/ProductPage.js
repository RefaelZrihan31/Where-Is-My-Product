...
export default function ProductPage({ route }) {
   ...
   // השמת טווח קבוע / ברירת מחדל עבור טווח המרחק
   ...
   //פעולה הפותחת את חלון בחירת טווח המרחק לצורך ניווט לסניף
   const rangeDialog = () => {
      setRangeDialogVisible(!rangeDialogVisible);
   };

   // פעולה א-סינכרונית לשליפת נתוני המיקום הנוכחיים של המשתמש לאחר קבלת אישור לשימוש בשירותי מיקום
   ...
   const haversineDistance = (mk1, mk1long, mk2, mk2long) => {
   ...
   // פעולה הבודקת ומחזירה את המחיר המינימלי שיש למוצר מבין כל החברות המוכרות אותו
   const lowestPrice = () => {
      let priceMin;
   ...
   useEffect(() => {
      ...
      let savedProduct = AsyncStorage.getItem(JSON.stringify(productToProductPage.UPC_Code));
   ...
   // פעולה א-סינכרונית לשמירת טווח החיפוש בזכרון המכשיר
   const storeRangeData = async (value) => {
      try {
         maxRadiusRange != 15
            ? await AsyncStorage.setItem("range", JSON.stringify(value))
            : await AsyncStorage.setItem(JSON.stringify("range"), JSON.stringify(15));
   ...
   //  פעולה א-סינכרונית השולפת את המידע מתוך זיכרון המכשיר - טווח החיפוש שנבחר
   ...
   // פעולה המנקה את זיכרון המכשיר - לשימוש הפיתוח בלבד
   ...
   // פעולה המקבלת קוד חברה וגם שם סניף\ מיקום ובודקת ומחזירה האם המוצר זמין באחד מסניפי החברות
   ...
   // פעולה המציגה את החברות, מחיר, זמינות בסניף עבור המוצר באופן ממוין מהמחיר הנמוך לגבוה
   const showList = () => {
      return companiesSellingProductAPIData
         .sort((fisrtProductPrice, secondProductPrice) => (fisrtProductPrice.Product_Price > secondProductPrice.Product_Price ? 1 : -1))
         .map((comp) => {
            return (
               <TouchableOpacity
   ...
   return (
      <SafeAreaView style={styles.container}>
         <ScrollView>
            <FAB icon='map-marker-distance' small color='rgb(4,207,146)' style={styles.fab} onPress={() => rangeDialog()} />
            <Dialog
               isVisible={rangeDialogVisible}
               onBackdropPress={rangeDialog}
